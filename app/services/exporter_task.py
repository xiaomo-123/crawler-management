        ...
        try:
            ...
            if self.cookie:
                ...
                try:
                    ...
                    # 确保expires字段是浮点数类型
                    if 'expires' in cookie_dict and isinstance(cookie_dict['expires'], str):
                        try:
                            cookie_dict['expires'] = float(cookie_dict['expires'])
                        except (ValueError, TypeError):
                            # 如果转换失败，删除expires字段或设置为默认值
                            cookie_dict.pop('expires', None)
                    storage_state = {
                        "cookies": [cookie_dict]
                    }
