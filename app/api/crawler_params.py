...
async def get_crawler_params(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    ...
    for param in params:
        result.append({
            ...
            "start_time": param.start_time.strftime("%Y-%m-%d %H:%M:%S") if param.start_time else None,
            "end_time": param.end_time.strftime("%Y-%m-%d %H:%M:%S") if param.end_time else None,
...
async def create_crawler_param(param_data: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if param_data.get("start_time"):
            start_time = datetime.strptime(param_data["start_time"], "%Y-%m-%d %H:%M:%S")

        if param_data.get("end_time"):
            end_time = datetime.strptime(param_data["end_time"], "%Y-%m-%d %H:%M:%S")
...
async def update_crawler_param(param_id: int, param_data: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if "start_time" in param_data:
            param.start_time = datetime.strptime(param_data["start_time"], "%Y-%m-%d %H:%M:%S") if param_data["start_time"] else None
        if "end_time" in param_data:
            param.end_time = datetime.strptime(param_data["end_time"], "%Y-%m-%d %H:%M:%S") if param_data["end_time"] else None
